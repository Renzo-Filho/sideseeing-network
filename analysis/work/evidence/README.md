...
Use [SP_METHOD_DECISIONS.md](../../../docs/sp/SP_METHOD_DECISIONS.md) for a human explanation. JSON is evidence/provenance, CSV holds summaries, and Parquet holds reusable allocation/overlay records. Preserve the contents and versioned manifests: the attribute pipeline consumes selected files from these experiments. New experiments should use new folders rather than overwrite accepted evidence.
