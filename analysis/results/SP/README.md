...
Read IDs with `dtype={"district_id": str}` to preserve leading zeros. Brás is `10`. No scaling, model fitting or similarity ranking has been performed. M5/U5 are excluded. U2 retains 508,844 unlocated jobs outside primary totals; the six M6 class shares are dependent. Review source/proxy metadata before modeling.
